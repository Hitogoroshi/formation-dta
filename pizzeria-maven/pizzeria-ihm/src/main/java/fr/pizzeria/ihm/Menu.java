...
public class Menu {
	...
	public Menu(IhmHelper helper) {
		 ...
		 * new Reflections(); Set<Class<?>> annotated =
		 * reflection.getTypesAnnotatedWith(AnnotationAction.class); // permet
		 * de manipuler des entier si j'ai plusieurs processus qui // arrivent
		 * en meme temps // je veux incrementer et je suis sure que cela vas
		 * marcher AtomicInteger yournotkillme = new AtomicInteger(0);
		 * 
		 * annotated.stream().forEach(uneClasse -> { try { Constructor<?>
		 * constructor = uneClasse.getConstructor(IhmHelper.class); Object
		 * action = constructor.newInstance(helper);
